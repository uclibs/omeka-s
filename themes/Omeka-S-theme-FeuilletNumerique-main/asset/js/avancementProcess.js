/*!
 * stackedBarChart v0.0.1 
 * Copyright 2021 Samuel Szoniecky
 * Licensed under MIT (https://github.com/twbs/bootstrap/blob/master/LICENSE)
 * merci à https://observablehq.com/@d3/stacked-normalized-horizontal-bar
 */
class avancementProcess {
    constructor(params) {
        var me = this;
        this.data = params.data ? params.data : [];
        this.cont = params.cont ? params.cont : 'body';
        ...
        this.init = function () {
            ...
            let etapes = me.data.columns.slice(1);
            let pivot = etapes.flatMap(etape => me.data.map(d => ({
                domaine: d.name,
                etape,
                fait: d[etape],
                process: d[etape]
            }))) // pivot
            chart = StackedBarChart(pivot, {
                x: d => d.process,
                y: d => d.domaine,
                z: d => d.etape,
                yDomain: d3.groupSort(
                    pivot,
                    (D) => D[0].process / d3.sum(D, d => d.process), // proportion
                    d => d.domaine // sort y by x
                ),
                zDomain: etapes,
                colors: d3.schemeSpectral[etapes.length],
                width:me.width
            })
